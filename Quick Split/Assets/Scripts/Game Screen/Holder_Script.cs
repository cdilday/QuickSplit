...
public class Holder_Script : MonoBehaviour {
	...
	public void getNextPiece()
	{
		...
		holder[0, 0].GetComponent<piece_script> ().lockPos = new Vector2 ( -1, transform.position.y);
		holder [0, 1] = holder [1, 1];
		holder[0, 1].GetComponent<piece_script> ().lockPos = new Vector2 ( 0, transform.position.y);
		holder [1, 0] = holder [2, 0];
		holder[1, 0].GetComponent<piece_script> ().lockPos = new Vector2 ( -1 - 2.5f, transform.position.y);
		holder [1, 1] = holder [2, 1];
		holder[1, 1].GetComponent<piece_script> ().lockPos = new Vector2 ( 0 - 2.5f, transform.position.y);
		...
		holder[2, 0].GetComponent<piece_script> ().locked = true;
		holder[2, 0].GetComponent<piece_script> ().inHolder = true;
		holder[2, 0].GetComponent<piece_script> ().lockPos = new Vector2 ( -1 - 2 * 2.5f, transform.position.y);
		holder[2, 1].GetComponent<piece_script> ().locked = true;
		holder[2, 1].GetComponent<piece_script> ().inHolder = true;
		holder[2, 1].GetComponent<piece_script> ().lockPos = new Vector2 ( 0 - 2 * 2.5f, transform.position.y);
	}
}
